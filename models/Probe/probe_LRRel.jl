function probe_LRRel(nactions, nstates)
    @model PROBEmodelLRRel(na, ns) = begin
        μ₀ ~ Normal()
        λ ~ Normal()
        κ ~ Beta()
        ...
        θ = 0.5 * logistic(Θ[2])
        η = logistic(Θ[3])
        return generate_latent(Val(:PROBEmodelLRRel), na, ns, μ₀ = μ₀,λ=λ, κ = κ, β = β, ϵ = ϵ, τ = τ, η = η, θ = θ, m = 1) 
    end

    mdl = PROBEmodelLRRel(nactions, nstates)
    @evolution mdl begin
        # Update reliability 
        ...
        α = logistic(λ * (logit(Reliabilities[Actor[1]]) - μ₀))
        for i = 1:na
            if i == a
...
function generate_latent(::Val{:PROBEmodelLRRel}, na, ns ; μ₀ = μ₀,λ=λ, κ, β, ϵ, τ, θ, η, m)
    ...
    return (μ₀ = μ₀,λ=λ, κ = κ, β = β, ϵ = ϵ, τ = τ, θ = θ, η = η,  m = m,
        ...
        Qactions = Vector{typeof(λ)}(fill(1/na, na)))
end